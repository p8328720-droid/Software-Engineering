        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr><td>#{{ str_pad($report->id, 5, '0', STR_PAD_LEFT) }}</td><td>{{ $report->title }}</td><td>{{ $report->facility->name }}</td><td>{{ $report->created_at->format('d/m/Y') }}</td><td><x-report-status :status="$report->status" /></td><td>@if($report->sla_deadline < now() && $report->status != 'completed')<span class="text-danger">Terlambat</span>@else<span class="text-success">On Track</span>@endif</td><td><a href="{{ route('mahasiswa.reports.show', $report) }}" class="btn btn-sm btn-outline-primary">Detail</a></td></tr>
                    @empty
