    ...
    <div class="card-body">
        ...
        <form action="{{ route('mahasiswa.reports.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="facility_id" class="form-select @error('facility_id') is-invalid @enderror" required>
                    ...
                    @if(isset($facilities) && $facilities->count() > 0)
                        <optgroup label="Lab">
                            @foreach($facilities->where('category', 'Lab') as $facility)
                        ...
                        <optgroup label="Kelas">
                            @foreach($facilities->where('category', 'Kelas') as $facility)
                                <option value="{{ $facility->id }}" {{ old('facility_id') == $facility->id ? 'selected' : '' }}>
                                    {{ $facility->name }} - {{ $facility->location }}
                                </option>
                            @endforeach
            ...
            <div class="alert alert-info">
                ...
                <strong>Informasi SLA:</strong> Laporan akan segera diproses oleh teknisi setelah dikirim.
            </div>
