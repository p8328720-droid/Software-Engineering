...
<div class="card border-0"><div class="card-header bg-white"><h5 class="mb-0"><i class="fas fa-tasks text-orange me-2"></i>Tugas Aktif</h5></div><div class="card-body"><div class="table-responsive"><table class="table table-hover"><thead><tr class="table-light"><th>No. Laporan</th><th>Judul</th><th>Lokasi</th><th>Urgensi</th><th>SLA Deadline</th><th>Aksi</th></tr></thead><tbody>@forelse($active_tasks as $task)<tr><td>#{{ str_pad($task->report_id, 5, '0', STR_PAD_LEFT) }}</td><td>{{ $task->report->title }}</td><td>{{ $task->report->location_detail }}</td><td><x-urgency-badge :urgency="$task->report->urgency" /></td><td>{{ $task->report->sla_deadline->format('d/m/Y H:i') }}</td><td><a href="{{ route('teknisi.tasks.show', $task) }}" class="btn btn-sm btn-primary">Kerjakan</a></td></tr>@empty<tr><td colspan="6" class="text-center py-4">Tidak ada tugas aktif</td></tr>@endforelse</tbody></table></div></div></div>
@endsection
