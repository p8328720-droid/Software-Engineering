...
<div class="card border-0"><div class="card-header bg-white"><h5 class="mb-0"><i class="fas fa-list-alt text-orange me-2"></i>Laporan Terbaru</h5></div><div class="card-body"><div class="table-responsive"><table class="table table-hover"><thead><tr class="table-light"><th>ID</th><th>Pelapor</th><th>Fasilitas</th><th>Judul</th><th>Status</th><th>Teknisi</th><th>SLA</th></tr></thead><tbody>@forelse($reports as $report)<tr><td>#{{ str_pad($report->id, 5, '0', STR_PAD_LEFT) }}</td><td>{{ $report->user->name }}</td><td>{{ $report->facility->name }}</td><td>{{ Str::limit($report->title, 30) }}</td><td><x-report-status :status="$report->status" /></td><td>{{ $report->technicianAssignments->first()?->technician->name ?? '-' }}</td><td>@if($report->sla_deadline < now() && $report->status != 'completed')<span class="text-danger">Terlambat</span>@else<span class="text-success">On Track</span>@endif</td></tr>@empty<tr><td colspan="7" class="text-center py-4">Belum ada laporan</td></tr>@endforelse</tbody></table></div></div></div>
@endsection
